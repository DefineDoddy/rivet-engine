...
void main() {
    if (mesh_material.transparent) {
        vec4 diffuse = texture(mesh_material.diffuse, frag_tex_coords);
...
vec3 calc_dir_light(Light light, vec3 normal, vec3 view_dir) {
    ...
    vec3 diffuse = brightness * light.colour * mesh_material.tint_colour * model_material.tint_colour;
...
vec3 calc_point_light(Light light, vec3 normal, vec3 view_dir) {
    ...
    float brightness = max(dot(normal, light_dir), 0.0) * light.intensity;
    vec3 diffuse = brightness * light.colour * mesh_material.tint_colour * model_material.tint_colour * attenuation;
...
vec3 calc_spot_light(Light light, vec3 normal, vec3 view_dir) {
    ...
    float brightness = max(dot(normal, light_dir), 0.0) * light.intensity;
    vec3 diffuse = brightness * light.colour * mesh_material.tint_colour * model_material.tint_colour * attenuation * spot_brightness;
