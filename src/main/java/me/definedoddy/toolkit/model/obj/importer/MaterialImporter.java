    ...
    private void parseMaterial(String startLine) {
        ...
        try {
            while (true) {
                ...
                if (line == null || line.startsWith("newmtl ")) break;
                String[] parts = line.split(" ");

                if (line.startsWith("Kd ")) {
                    builder.colour(new Color(Float.parseFloat(parts[1]), Float.parseFloat(parts[2]), Float.parseFloat(parts[3])));
                } else if (line.startsWith("Ns ")) {
                    builder.shininess(Float.parseFloat(parts[1]));
                } else if (line.startsWith("d ")) {
                    if (Float.parseFloat(parts[1]) > 0.5) builder.transparent();
