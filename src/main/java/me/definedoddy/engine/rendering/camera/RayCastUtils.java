...
public class RayCastUtils {
    public Vector3f screenToWorldPoint(float x, float y) {
    ...
    public Vector3f eyeToWorldPoint(Vector4f eyeCoords) {
        Matrix4f invertedView = new Matrix4f(Camera.get().getViewMatrix()).invert();
        Vector4f rayWorld = invertedView.transform(eyeCoords, null);
        Vector3f rayPoint = new Vector3f(rayWorld.x, rayWorld.y, rayWorld.z);
        rayPoint.normalize();
        return rayPoint;
    }

    public Vector4f clipToEyeCoords(Vector4f clipCoords) {
    ...
    public Vector2f screenToNormalisedDeviceCoords(float screenX, float screenY) {
