...
public class MeshBuilder {
    ...
    public Mesh build() {
        for (int i = 0; i < vertexPositions.length; i++) {
            Vector2f textureCoord = textureCoords.length > i ? textureCoords[i] : new Vector2f();
            Vector3f normal = normals.length > i ? normals[i] : new Vector3f();

            mesh.addVertex(new Vertex(i, vertexPositions[i], textureCoord, normal));
        }
