...
void main() {
    if (meshMaterial.transparent) {
        vec4 diffuse = texture(meshMaterial.diffuse, frag_tex_coords);
...
vec3 calc_dir_light(Light light, vec3 normal, vec3 view_dir) {
    ...
    vec3 diffuse = brightness * light.colour * meshMaterial.tint_colour * modelMaterial.tint_colour;

    vec3 specular = calc_specular(light, normal, view_dir);

    if (meshMaterial.has_diffuse) {
        diffuse *= texture(meshMaterial.diffuse, frag_tex_coords).rgb;
    }

    if (meshMaterial.has_specular) {
        specular *= texture(meshMaterial.specular, frag_tex_coords).r;
    }
...
vec3 calc_point_light(Light light, vec3 normal, vec3 view_dir) {
    ...
    float brightness = max(dot(normal, light_dir), 0.0) * light.intensity;
    vec3 diffuse = brightness * light.colour * meshMaterial.tint_colour * modelMaterial.tint_colour * attenuation;
...
vec3 calc_spot_light(Light light, vec3 normal, vec3 view_dir) {
    ...
    float brightness = max(dot(normal, light_dir), 0.0) * light.intensity;
    vec3 diffuse = brightness * light.colour * meshMaterial.tint_colour * modelMaterial.tint_colour * attenuation * spot_brightness;
...
vec3 calc_specular(Light light, vec3 normal, vec3 view_dir) {
    ...
    return pow(specular_factor, SHINE_DAMPER) * light.colour * meshMaterial.shininess * modelMaterial.shininess;
}